...
if(!wp_verify_nonce($nonce, 'login_me_nonce')){
	wp_send_json_error(array(
			...
			'redirect' => false
		)); // проверим её специальной функцией, а если строки не совпадут отправляем json ответ с ошибкой, функция wp_send_json_error сама прекратит работу скрипта
}
...
if(!$log){
	wp_send_json_error(array(
			...
			'redirect' => false
		)); // если что то из полей пусто - ошибка
}
...
if(!$user){
	$user = get_user_by('email', $log); // если там пусто, значит попробуем получить юзера по мылу
}
...
if(!$user){
	wp_send_json_error(array(
			...
			'redirect' => false
		)); // если в обоих случаях пустота, значит такого юзера нет - возвращаем ошибку и умираем
}
/*
if(get_user_meta($user->ID, 'has_to_be_activated', true) != false){
	wp_send_json_error(array(
			...
			'redirect' => false
		)); // закомментить если не используется активация, см. след. статьи
}
*/
...
if(is_wp_error($user)){
	wp_send_json_error(array(
			'message' => 'Ошибочное логин/email или пароль.',
			'redirect' => false
		)); // если вернулся объект с ошибкой  - умираем и пешем ошибку, уточнять не будем
}
else{


	wp_send_json_success(array(
			'message' => 'Hello ' . $user->display_name . '. Loading ...',
